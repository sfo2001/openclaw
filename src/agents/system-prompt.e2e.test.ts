...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes current time when includeTimeInPrompt is true", () => {
    const prompt = buildAgentSystemPrompt({
    ...
    expect(prompt).toContain("## Current Date & Time");
    expect(prompt).toContain("Current time: Monday, January 5th, 2026 — 15:26");
    expect(prompt).toContain("Time zone: America/Chicago");
  });

  it("omits current time when includeTimeInPrompt is false", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/clawd",
      userTimezone: "America/Chicago",
      userTime: "Monday, January 5th, 2026 — 15:26",
      userTimeFormat: "24",
      includeTimeInPrompt: false,
    });
  ...
  it("omits current time when includeTimeInPrompt is not set", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/clawd",
      userTimezone: "America/Chicago",
      userTime: "Monday, January 5th, 2026 — 15:26",
